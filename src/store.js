...
export default new Vuex.Store({
  ...
  getters: {
    entityCollectionGet: (state) => (entityId) => {
      return state.entityCollection[entityId]
    },
    entityList(state) {
    ...
    entity: (state, getters) => (type) => {
      return find(getters.entityList, ['type', type])
    },
    enumTypes(state) {
      if (!state.metadata) return;
      return Array.prototype.map.call(state.metadata.querySelectorAll("[Namespace*='Rehab'] EnumType"), object => {
        return {
          type: object.getAttribute('Name'),
          options: Array.prototype.map.call(object.querySelectorAll('Member'), prop => {
            return {name: prop.getAttribute('Name'), value: prop.getAttribute('Value')}
          })
        }
      })
    }
  ...
  actions: {
    entityCollectionFetch({commit, state}, {url, entityId}) {
      return axios.get(url).then(({data}) => {
        commit('entityCollectionUpdate', {data: data.value, entityId})
      })
    },
  ...
  mutations: {
    ...
    entityCollectionUpdate(state, {data, entityId}) {
      Vue.set(state.entityCollection, entityId, data)
    },
  },
})